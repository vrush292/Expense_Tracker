...
function showSuccess() {
    ...
    const successTick = document.getElementById('success-tick-container');
    
    // Apply blur effect to the entire form
    formContainer.classList.add('blur');
    
    // Show the success tick animation container
    successTick.classList.add('show');
    
    // Optional: Remove the blur effect and success tick after a short delay (for example, 2 seconds)
    setTimeout(() => {
        formContainer.classList.remove('blur');
        successTick.classList.remove('show');
    }, 2000); // The success tick stays for 2 seconds before disappearing
}
...
document.querySelectorAll('form').forEach(form => {
    form.addEventListener('submit', (e) => {
        ...
        // Call showSuccess after a successful form submission
        showSuccess();
